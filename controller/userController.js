...
// to handle view==create ,update, index
...
const createUser = async (req,res) => {
    try {
        ...
        const extUserName = await User.findOne({username: req.body.username})
          ...
          res.status(200).json({ data: newUser, msg: "New user Added succesfully"}) //data read with froent end
    }catch(err) {
        res.status(500).json({ msg: err.message })
    }
}
...
const deleteUser = async (req,res) => {
    try {
        ...
        return res.status(200).json({ msg:`hi ${extUser.name},your datahas been removed`})    
